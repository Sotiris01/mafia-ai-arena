...
- [X] `src/engine/BalanceCalculator.ts` — calculateBalance(), countByFaction(), getStage(), getLosing()
- [X] Minimal `src/hooks/useGameLoop.ts` — startGame(playerCount), resetGame(), exposes gameState + humanPlayer + playerIds

### Step 2 — Shared UI components

- [X] `src/components/shared/PlayerAvatar.tsx` — player icon with alive/dead indicator
- [X] `src/components/shared/RoleCard.tsx` — flip-to-reveal role card (name, alignment, ability)
- [X] `src/components/shared/Timer.tsx` — countdown timer (used across phases)
- [X] `src/components/shared/FactionBanner.tsx` — Town/Mafia/Neutral colored banner

### Step 3 — Lobby screen (real)

- [X] `src/components/lobby/PlayerCount.tsx` — player count slider (7–16)
- [X] `src/components/lobby/RolePreview.tsx` — shows which roles will be in the game by tier
- [X] `src/components/lobby/StartButton.tsx` — animated start button
- [X] `app/index.tsx` — wire lobby components + useGameLoop.startGame → show RoleCard → navigate to /game/day

### Step 4 — Chat system (human only)

- [X] Minimal `src/hooks/useChat.ts` — sendMessage(text), messages list, addMessage to ChatState (public channel only)
- [X] `src/components/chat/ChatBubble.tsx` — single message bubble with player avatar
- [X] `src/components/chat/ChatInput.tsx` — text input + send button
- [X] `src/components/chat/PublicChat.tsx` — scrollable message list (FlatList) + ChatInput
- [X] `app/game/day.tsx` — wire useChat + PublicChat, human can type messages and see them appear
...
- [X] `__tests__/engine/BalanceCalculator.test.ts` — 15 tests: countByFaction, calculateBalance, getStage, getLosing
- [X] `__tests__/hooks/useGameLoop.test.ts` — 17 tests: initial state, startGame, resetGame, state module resets
- [X] `__tests__/hooks/useChat.test.ts` — 13 tests: sendMessage, channels, trimming, empty messages, sequential IDs
- [X] `__tests__/contexts/GameContext.test.tsx` — 7 tests: provider requirement, startGame/resetGame through context
